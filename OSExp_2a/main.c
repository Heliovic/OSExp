...
int alg;    // 选择算法，1为EDF，2为RMS
int curr_proc = -1;
int demo_time = 100;    // 演示时间

task* tasks;
pthread_mutex_t proc_wait[100]; // 任务锁，显然，task_num 应不超过 100
...
int main(int argc, char** argv)
{
    ...
    scanf("%d", &task_num);
    tasks = (task*)malloc(task_num * sizeof(task));     // 分配内存空间
    int i;
...
void proc(int* args)
{
    while(tasks[*args].ci_left > 0)
    {
        ...
        if (tasks[*args].ci_left == 0)   // 一次执行完毕
        {
